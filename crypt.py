...
def read_public_key(filename):
    """Read RSA public key from file; expects two lines: n and e."""
...
def read_private_key(filename):
    """Read RSA private key from file; expects two lines: n and d."""
    with open(filename, "r") as f:
        lines = f.read().splitlines()
        n = int(lines[0])
        d = int(lines[1])
...
def encrypt_file(pubkey_file, input_file, output_file):
    ...
    # Store the RSA-encrypted AES key and AES ciphertext (both hex-encoded) in JSON.
    output_data = {
